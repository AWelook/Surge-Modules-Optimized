...
7. Update only the affected project under `modules/`, `scripts/`, `converted/`, `upstream/`, its registry metadata, and directly related tests or automation. If its published module is listed in `COMBINED_SOURCES` in `scripts/build-combined-module.mjs`, run `npm run build:combined` and include the regenerated `modules/ad/ad-combined.sgmodule` in the same commit. The derived combined module is considered a directly related file.
8. Add behavior tests for every added or changed rule. Run `npm test`, which must also pass the combined-module freshness check, check the final diff, and clearly state any validation that could not be performed, such as live Surge traffic replay.
